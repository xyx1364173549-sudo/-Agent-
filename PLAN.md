...
- [x] M5.1 应用工厂与路由分层 — 交付：`src/api/app.py`、`routers/`
- [x] M5.2 会话与对话接口 — 交付：SSE 流式对话端点
- [x] M5.3 记忆 · 画像 · 进度查询接口 — 交付：状态可见性接口
- [x] M5.4 服务层测试 — 交付：TestClient 用例全绿

## [M6] 前端交互界面
<!-- section: 第 6 章 -->

- [x] M6.1 页面骨架 — 交付：对话区 + 学习路径图 + 记忆面板三栏布局
- [x] M6.2 后端联调 — 交付：SSE 流式渲染接通真实接口
- [x] M6.3 视觉打磨 — 交付：深色主题、响应式、加载态与异常态

## [M7] 实验与评估 · 论文 3 组实验
<!-- section: 第 6 章（3 组实验） -->

- [x] M7.1 实验数据集与指标定义 — 交付：测试集 + 评价指标（记忆命中率、路径合理性、回答质量）
- [x] M7.2 实验一：分层记忆消融 — 交付：无记忆 / 仅工作记忆 / 全分层记忆 对比
- [x] M7.3 实验二：切分与检索策略对比 — 交付：三种切分 × 三种检索 交叉实验
- [x] M7.4 实验三：动态规划 vs 静态路径 — 交付：学习效率与路径适应性对比
- [x] M7.5 结果图表与实验报告 — 交付：论文可用图表 + 实验章节素材
...
| 2026-09-17 | **M5 服务层全部完成（4/4）**：应用工厂 + 路由分层（`src/api/`）、SSE 流式开课（实测 241 个 token 事件逐字推送）、作答与状态查询、记忆 / 画像 / 知识库接口，29 条 TestClient 用例全绿。**修复 SQLite 跨线程限制**（`check_same_thread=False`）与「路径掌握度返回过期快照」两个真实缺陷 |
| 2026-09-17 | **M6 前端全部完成（3/3）**：`web/` 三栏界面（对话区 / 学习路径 / 记忆面板），原生 HTML+CSS+JS 无构建步骤，由后端托管实现前后端同源；SSE 逐字渲染讲解并处理代码块。真实浏览器实测：路径卡片 8 个、讲解 404 字、批改与记忆面板同步更新、**无 JS 报错**。修复「红色错误条常驻」（CSS 的 `display` 覆盖了 `[hidden]`）与「开课后记忆面板不刷新」 |
| 2026-09-17 | **M7 实验与评估全部完成（5/5）**：实验一记忆消融（无记忆 0.44 → 仅工作 0.67 → 全分层 0.89，单调递增）；实验二切分×检索（关键词 > 混合 > 向量，根因是本地英文向量模型）；实验三动态 vs 静态（目标均分 +166%，有效练习 50%→99.9%）。实验三揪出**画像不衰减的真实缺陷**并修复（读取时按 30 天半衰期衰减 + 可注入时钟）。matplotlib 生成论文图表，`docs/experiments/REPORT.md` 汇总三组实验 |
