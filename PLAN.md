...
- [ ] M3.6 RAG 测试 — 交付：检索与重排单测全绿

## [M4] 学习者画像 · 动态规划 · 多 Agent 编排
<!-- section: 第 5 章（创新点 2） -->

- [ ] M4.1 学习者画像模型 — 交付：`src/planning/profile.py`，掌握度/目标/偏好 + 持久化
- [ ] M4.2 目标拆解器 — 交付：`src/planning/decomposer.py`，把学习目标拆为可执行知识点
- [ ] M4.3 路径规划器 — 交付：`src/planning/planner.py`，产出带依赖关系的学习路径
- [ ] M4.4 三类功能 Agent — 交付：导师 Agent / 出题 Agent / 评估 Agent，`src/agents/`
- [ ] M4.5 LangGraph 状态图编排 — 交付：条件路由 + 循环 + 状态持久化，`src/planning/graph.py`
- [ ] M4.6 动态重规划 — 交付：依据评估结果实时调整后续路径
- [ ] M4.7 端到端串联与测试 — 交付：完整学-练-评-调闭环 demo + 测试全绿
